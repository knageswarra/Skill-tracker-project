...
import { Ping } from 'src/app/model/ping';
...
export class AddAssociateComponent implements OnInit {
  ...
  setassociateinfo(response: Associate[] | undefined) {
    if (response) {
    this.Associateinfo = response;
  }
  }
  ...
  // }
  onRemoveconfirm(associateEmail: string) {
  ...
  sendemail(email:string){
    this.ping.emailAddress=email
    this.AssociateService.ping(this.ping).subscribe(response=>{
      console.log(response)
      alert(
